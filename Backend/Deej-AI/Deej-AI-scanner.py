...
def walkmp3s(paths):
    for path in paths:
        if os.path.isfile(path):
            if path.lower().endswith(('.flac', '.mp3', '.m4a')):
                yield os.path.basename(path), os.path.abspath(path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scan', type=str, nargs='+', help='Directory of MP3s and M4As to scan')
    ...
    for filepath in list(mp3tovecs.keys()):
        if not os.path.isfile(filepath):
            print("deleted")
            os.remove(dump_directory + "/" +os.path.splitext(filepath)[0].replace('\\', '_').replace('/', '_').replace(':','_') + '.p')
            del mp3tovecs[filepath]
